...
* position : 요소의 위치 지정 기준!!!
  ...
  * absolute : 위치 상 부모 요소를 기준
    ...
    * 위치 상이란?? 부모요소에 position : relative를 주지 않으면 뷰포트가 부모가 됨. 부모요소에  relative가 부여되어있지 않으면 static이라는 것인데, 이때는 더 상위 부모를 찾아가게되고 결국 최상위 부모인 뷰포트로 찾아가게됨
...
* transform: 변환함수1 변환함수2 변환함수3 .... ;
  ...
  원근법, 인동, 크기, 회전, 기울임 등 변환함수들 사용 가능
